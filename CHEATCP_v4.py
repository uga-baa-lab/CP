...
matfiles = os.path.join(results_dir,'matfiles')
defaults = cf.define_defaults()
master = os.path.join(results_dir,'UpperLimb_Mastersheet_OK_04-23-22.xlsx') #'UpperLimb_Mastersheet_OK_11-30-21.xlsx')
...
if os.path.isfile(os.path.join(results_dir,'all_df.pkl')): 
    #load it
    print('pickle files already exist, loading them. If you want to reprocess re-name these files')
    all_df = pd.read_pickle(os.path.join(results_dir,'all_df.pkl'))
    
    with open(os.path.join(results_dir,'allTrajs.pkl'), 'rb') as handle:
        allTrajs = pkl.load(handle)    


else:
    print('pickle files not found, creating them')
    [all_df,allTrajs] = cf.getDataCP(mdf,matfiles,defaults)
    all_df.to_pickle(os.path.join(results_dir,'all_df.pkl'))

    with open(os.path.join(results_dir,'allTrajs.pkl'), 'wb') as handle:
        pkl.dump(allTrajs, handle, protocol=pkl.HIGHEST_PROTOCOL)
...
with PdfPages(os.path.join(results_dir,'Trajs_'+plotsubject+plotday+'.pdf')) as pdf:
    ...
    #trajs= list(range(10,20))
        ...
        pdf.savefig(fig)
        plt.close()
####the_table[0,-1].set_height(0.3) #fix the cell size

   # fig.savefig(os.path.join(results_dir,'test2.pdf'))


#    axs[row,columns[i]].text(0.01, 0.95, moretext,
#            verticalalignment='top', horizontalalignment='left',
#            transform=axs[0].transAxes,
#            color=correct, fontsize=10)
...
circle2 = plt.Circle((traj['CursorX'][ft],traj['CursorY'][ft]), 5, color='m')
...
for x,trajx in enumerate(trials_to_plot):
    ...
    plt.plot(traj['CursorX'],traj['CursorY'])
    plt.plot(traj['CursorX'][499],traj['CursorY'][499],'bo')
...
totalids = 63 #last number of IDs used
