...
def getHandKinematics(thisData, defaults, i, subject):
    ...
    if not np.isnan(kinData['CT']) and kinData['RT'] < kinData['CT']:
        ...
        calculate_initial_movement_direction_error(kinData, HandX_filt, HandY_filt, xTargetPos, yTargetPos, i,subject)
...
def calculate_initial_movement_direction_error(kinData, HandX_filt, HandY_filt, xTargetPos, yTargetPos,i,subject):
    """
    Calculate Initial Movement Direction Error (IDE).
    ...
    # plots.plot_trial_IDE(kinData, HandX_filt, HandY_filt, xTargetPos, yTargetPos, theta_deg, i,subject)
...
def calculate_movement_time(kinData, feedbackOn, CursorY, yTargetPos):
    ...
    if not np.isnan(kinData['RT']):
        ...
        findCT = np.where(CursorY > yTargetPos[0])[0]
...
def getDataCP(mdf, matfiles, defaults):
    ...
    allTrajs = {}
    row_name_str=  ["cpvib040","cpvib042"]
