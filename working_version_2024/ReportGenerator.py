...
from Config import BASE_DIR,REPORT_RESULTS_DIR,data_file_location
...
def save_grouped_data(df, group_cols, filename):
    ...
    grouped_df_means.to_csv(os.path.join(REPORT_RESULTS_DIR, filename), index=False)
...
def process_and_save_day_data_std(df_stds, df_std_dur, day_num, exceltitle, all_ids):
    ...
    VARLIST = ['Accuracy', 'MT', 'RT', 'pathlength', 'velPeak', 'EndPointError', 'IDE', 'PLR', 'IE','targetDist_Hit_Interception']
...
def save_grouped_data_std(df, group_cols, filename):
    ...
    grouped_df.to_csv(os.path.join(REPORT_RESULTS_DIR, filename), index=False)  # Flatten MultiIndex columns
    return grouped_df
...
def main_std():
    ...
    df1_stds[['subject', 'group', 'day', 'Condition', 'Affected'] + VARLIST].to_csv(os.path.join(REPORT_RESULTS_DIR, 'Day1_stds_bysubject.csv'), index=False)
    ...
    df1_wide.sort_values(['group', 'KINARM_ID'], ascending=True).to_csv(os.path.join(REPORT_RESULTS_DIR, 'Day1_stds_bysubject_wide.csv'), index=False)

    # Excel Sheet Creation for Multiple Days
    exceltitle = os.path.join(REPORT_RESULTS_DIR, 'UL_KINARM_Mastersheet_Auto_Format_stds.xlsx')
    exceltitle2 = os.path.join(REPORT_RESULTS_DIR, 'UL_KINARM_Mastersheet_Long_Format_stds.xlsx')
...
def main_means():
    all_df = pd.read_csv(data_file_location)
    print(f'IE Values > 1000 are : {all_df.loc[all_df["IE"].abs() > 1000, "IE"]}')
    all_df.loc[all_df['IE'].abs() > 1000, 'IE'] = np.nan
    print(f'IE Values > 1000 are : {all_df.loc[all_df["IE"].abs() > 1000, "IE"]}')

    print(f'Columns in the data: {all_df.columns}')
    all_df = calculate_additional_columns(all_df)
    ...
    df1_means[['subject', 'group', 'day', 'Condition', 'Affected'] + VARLIST].to_csv(os.path.join(REPORT_RESULTS_DIR, 'Day1_means_bysubject.csv'), index=False)

    
    df1_wide = pivot_data_to_wide(df1_means, ['subject', 'studyid', 'group', 'day'], ['Condition', 'Affected'], VARLIST)
    # df1_wide.sort_values(['group', 'KINARM_ID'], ascending=True).to_csv(os.path.join(REPORT_RESULTS_DIR, 'Day1_means_bysubject_wide.csv'), index=False)

    # Excel Sheet Creation for Multiple Days
    exceltitle = os.path.join(REPORT_RESULTS_DIR, 'UL_KINARM_Mastersheet_Auto_Format_means.xlsx')
    exceltitle2 = os.path.join(REPORT_RESULTS_DIR, 'UL_KINARM_Mastersheet_Long_Format_means.xlsx')
...
def main_std_ide():
    ...
    exceltitle = os.path.join(REPORT_RESULTS_DIR, 'UL_KINARM_Mastersheet_Only_IDE_STD.xlsx')
...
def main_means_ide():
    ...
    exceltitle = os.path.join(REPORT_RESULTS_DIR, 'UL_KINARM_Mastersheet_Only_IDE_means.xlsx')
