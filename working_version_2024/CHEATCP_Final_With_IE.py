...
def getHandKinematics(thisData, defaults, i, subject):
    ...
    HandX_filt, HandY_filt, velX_filt, velY_filt = utils.filter_hand_data(HandX, HandY, velX, velY, defaults)
    handspeed = utils.calculate_handspeed(velX_filt, velY_filt)
    kinData['HandX_filt'] = HandX_filt
    kinData['HandY_filt'] = HandY_filt  
...
def calculate_position_and_distances(kinData, xTargetPos, yTargetPos, CursorX, HandX_filt, HandY_filt):
    ...
    kinData['targetDist_Hit_Interception'] = np.nan
    # Filter and store targetDist for Hit and Interception trials
    if kinData['Accuracy'] == 1 and kinData['Condition'] == 'Interception':
        kinData['targetDist_Hit_Interception'] = kinData['targetDist']
    ...
    for index, row in mdf.iterrows():
        ...
        for i in range(len(data)):
            ...
            row_values = [kinData['Condition'], thisData.T[16][0], thisData.T[11][0],
                          ...
                          kinData['EndPointError'], kinData['IDE'], kinData['PLR'], kinData['isCurveAround'], i, kinData['HandX_filt'].tolist(), kinData['HandY_filt'].tolist()]
        ...
        df = pd.DataFrame(allTrials, columns=['Condition', 'Affected', 'TP', 'Duration', 'Accuracy', 'FeedbackTime',
                                              ...
                                              'RTalt', 'IA_RTalt', 'maxpathoffset', 'meanpathoffset', 'xTargetEnd', 'yTargetEnd', 'EndPointError', 'IDE', 'PLR', 'isCurveAround','trial_number','HandX_filt','HandY_filt'])
        # data cleaning
        print('DF of Condition:', df['Condition'].unique())
